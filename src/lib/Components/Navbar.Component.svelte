	...
	>
		<NavLi href="/" class={activeUrl === '/' ? activeClass : ''}>
			<div class="flex gap-2 justify-center items-center">
				...
				<div class={activeUrl === '/' ? 'flex' : 'flex md:hidden'}>
					<p>{$_('home')}</p>
				</div>
			</div>
		</NavLi>
		...
		<NavLi href="/setting" class={activeUrl.startsWith('/setting') ? activeClass : ''}>
			<div class="flex gap-2 justify-center items-center">
				<svg
					class="w-6 h-6"
					fill="none"
	...
	<div class="hidden md:flex gap-2 justify-center items-center">
		<div class="w-auto h-10 rounded-lg bg-[#f1f1f1] dark:bg-blue flex justify-center items-center">
			...
			>
				...
				>
					<!-- svelte-ignore a11y-missing-attribute -->
					<img class="w-8 h-8 object-cover rounded-full" src="/images/kurdish.svg" />{$_('kurdish')}
				</DropdownItem>
			</Dropdown>
		</div>
