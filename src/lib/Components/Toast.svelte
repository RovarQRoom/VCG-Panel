...
{#if $toastStore.visible}
  <Toast
    class="fixed bottom-5 right-5 z-50 w-full max-w-xs p-4 text-gray-500 bg-white rounded-lg shadow dark:bg-gray-800 dark:text-gray-400"
    color={$toastStore.type === 'error' ? 'red' : $toastStore.type === 'success' ? 'green' : $toastStore.type === 'warning' ? 'yellow' : 'blue'}
    dismissable
    on:dismiss={() => toastStore.showToast('', 'default', 0)}
  >
