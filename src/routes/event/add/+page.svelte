<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			...
			createEvent.place = placeResponse.id;
			createEvent.date = new Date(dateTime).toISOString();
			createEvent.ticket = ticketType;

			const response = await eventStore.insert(createEvent);
